...
- **Limite de Dispositivos**: máx. 2 dispositivos por usuário; tabela `device_sessions` rastreia dispositivo (UUID em localStorage), nome do aparelho (auto-detectado), último acesso; admin pode ver e revogar dispositivos na página de Gerenciar Usuários; verificação ocorre no login e restauração de sessão
